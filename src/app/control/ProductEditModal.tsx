...
const isRefNoList = (keyName: keyof ProductPrisma, val: unknown): val is ProductPrisma['ref_no'] => keyName === 'ref_no'
    ...
    <Flex gap="8" w="full">
        {Object.keys(defaultTableValConf[keyName]).map((k) => (
            <Input
                ...
                onChange={(e) => setInputValue((val: any) => ({ ...(val || {}), [k]: e.target.value }))}
            />
        ))}
    </Flex>
)
...
const TableInput = <T extends TableValKey>({ keyName, item, setItem }: TableInputProps<T>) => {
    ...
    const multiInputKeyConf: KeyNameConf = {
        ref_no: {
            renderInput: () => typeof inputValue !== 'string' && multiInputLine(inputValue, 'ref_no', setInputValue),
        },
    }
                ...
                <Table.Row>
                    <Table.Cell width="100%">
                        {keyName in multiInputKeyConf ? (
                            <Flex gap="8" w="full">
                                {multiInputKeyConf[keyName]?.renderInput()}
                            </Flex>
                        ) : (
                            <Input value={inputValue as string} onChange={(e) => setInputValue(e.target.value)} />
                        )}
                    </Table.Cell>
                    <Table.Cell textAlign="end">
...
export const ProductEditModal = ({ buttonText, defaultItem, onSave }: ProductEditModalProps) => {
    ...
    const searchOptionsCollection = useMemo(
        () =>
            createListCollection({
    ...
    /** 基础文本输入框封装 */
    const TextEdit = useCallback(
        ({ keyName: key, item }: TextEditProps) => (
            <Input
                value={item[key] || ''}
                onChange={(e) => {

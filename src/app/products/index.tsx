...
export function ProductsClientContainer({ types, types_en, lang, tab, products }: Props) {
    ...
    const toggleFilter = (filterKey: string) => {
        setFilters((prev) => (prev.includes(filterKey) ? prev.filter((f) => f !== filterKey) : [...prev, filterKey]))
    ...
    const transformedProducts = useMemo(
        () =>
            products.map((p) => {
                ...
                // 获取对应位置的英文类型（如果存在）
                const englishType = typeIndex !== -1 && typeIndex < types_en.length ? types_en[typeIndex] : p.type // 如果找不到对应的英文类型，使用中文类型作为后备
                        ...
                        <>
                            <Grid templateColumns="repeat(auto-fill, minmax(250px, 1fr))" gap={4}>
                                {paginatedProducts.map((product) => {
                                    const categoryText = filterOptions.find((opt) => opt === product.category) || ''
